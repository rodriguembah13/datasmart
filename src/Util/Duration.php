...
class Duration
{
    ...
    protected function parseColonFormat(string $duration): int
    {
        ...
        if (count($parts) < 2 || count($parts) > 3) {
            throw new \InvalidArgumentException(sprintf('Invalid colon format given in "%s"', $duration));
        }

        foreach ($parts as $part) {
            if (0 === strlen($part)) {
                throw new \InvalidArgumentException(sprintf('Colon format cannot parse "%s"', $duration));
            }
            if (((int) $part) < 0) {
                throw new \InvalidArgumentException(sprintf('Negative input is not allowed in "%s"', $duration));
            }
        }
